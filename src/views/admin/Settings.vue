        ...
        <v-row class="mt-4">
          <v-col cols="12">
            <v-card variant="outlined" class="rounded-xl overflow-hidden">
              <v-card-title class="bg-indigo-lighten-5 text-indigo-darken-3 d-flex align-center py-3 px-4">
                <v-icon class="mr-2" color="indigo">mdi-shield-account-outline</v-icon>
                <span class="text-h6 font-weight-bold">Role Management & Access Levels</span>
              </v-card-title>
              <v-card-text class="pa-4">
                <v-row>
                  <!-- Admin -->
                  <v-col cols="12" md="6" lg="3">
                    <v-card variant="tonal" color="primary" class="h-100 rounded-lg pa-3">
                      <div class="d-flex align-center justify-space-between mb-2">
                        <span class="text-subtitle-1 font-weight-bold">Admin</span>
                        <v-chip color="primary" size="x-small" variant="flat">Full Access</v-chip>
                      </div>
                      <div class="text-caption text-medium-emphasis">
                        Full access to all congregation modules: Overview, Publishers List (Add/Edit/Delete/Import/Export), Groups & Order, Emergency Contacts, All Reports & Analytics, Schedules, Territory Management, Settings & User Management.
                      </div>
                    </v-card>
                  </v-col>

                  <!-- Editor (Ministerial Servant) -->
                  <v-col cols="12" md="6" lg="3">
                    <v-card variant="tonal" color="blue" class="h-100 rounded-lg pa-3">
                      <div class="d-flex align-center justify-space-between mb-2">
                        <span class="text-subtitle-1 font-weight-bold">Editor (Ministerial Servant)</span>
                        <v-chip color="blue" size="x-small" variant="flat">MS Group Editor</v-chip>
                      </div>
                      <div class="text-caption text-medium-emphasis">
                        • Home & Congregation Overview: View-only (cards non-clickable), last month summary for assigned Group only.<br>
                        • Publishers List: View-only for assigned Group (no Add/Import/Edit).<br>
                        • Groups List: View assigned groups (no Edit Order; Groups menu hidden).<br>
                        • Emergency Contacts: View-only for assigned Group.<br>
                        • Reports: View Publisher Records (Edit disabled).<br>
                        • Territory: View Territory List for assigned Group (Overview & S-13 hidden unless appointed Territory Assistant).
                      </div>
                    </v-card>
                  </v-col>
                  ...
                  <!-- Editor (Publisher) -->
                  <v-col cols="12" md="6" lg="3">
                    <v-card variant="tonal" color="teal" class="h-100 rounded-lg pa-3">
                      <div class="d-flex align-center justify-space-between mb-2">
                        <span class="text-subtitle-1 font-weight-bold">Editor (Publisher)</span>
                        <v-chip color="teal" size="x-small" variant="flat">Publisher Editor</v-chip>
                      </div>
                      <div class="text-caption text-medium-emphasis">
                        • Home & Congregation Overview: View-only (cards non-clickable), last month summary for assigned Group.<br>
                        • Publishers List: Completely hidden.<br>
                        • Groups List: View assigned groups (no Edit Order; Groups menu hidden).<br>
                        • Emergency Contacts: View-only for assigned Group.<br>
                        • Reports: View Publisher Records (Edit disabled).<br>
                        • Territory: View Territory List for assigned Group (Overview & S-13 hidden unless appointed Territory Assistant).
                  ...
                  <!-- Viewer -->
                  <v-col cols="12" md="6" lg="3">
                    <v-card variant="tonal" color="grey" class="h-100 rounded-lg pa-3">
                      <div class="d-flex align-center justify-space-between mb-2">
                        <span class="text-subtitle-1 font-weight-bold">Viewer</span>
                        <v-chip color="grey-darken-1" size="x-small" variant="flat">Read Only</v-chip>
                      </div>
                      <div class="text-caption text-medium-emphasis">
                        Read-only access to meeting schedules and non-sensitive congregation information. Cannot view publisher personal records, reports, or settings.
                      </div>
                    </v-card>
                  </v-col>
                </v-row>
      ...
      <v-card class="rounded-xl overflow-hidden">
        ...
        <v-card-text class="pa-4">
          <v-form ref="userFormRef" v-model="isUserFormValid" @submit.prevent="saveUser">
            ...
            <v-select v-model="userForm.role" :items="appRoles" label="Access Level" variant="outlined" class="mb-2" />

            <!-- Role Checkboxes with Access Details -->
            <div class="mb-4">
              ...
              <div class="d-flex flex-column ga-2">
                ...
                <v-card
                  ...
                  :class="{ 'border-primary bg-blue-lighten-5': activeRolePreset === 'admin' }"
                  @click="applyRolePreset('admin')"
                >
                  <div class="d-flex align-start">
                    <v-checkbox-btn
                      :model-value="activeRolePreset === 'admin'"
                      color="primary"
                      density="compact"
                      class="mt-0 pt-0 mr-2"
                      @update:model-value="applyRolePreset('admin')"
                    />
                    <div class="flex-grow-1">
                      <div class="d-flex align-center justify-space-between">
                        <span class="text-subtitle-2 font-weight-bold">Admin</span>
                        <v-chip size="x-small" color="primary" variant="flat">Full Access</v-chip>
                      </div>
                      <div class="text-caption text-medium-emphasis mt-1">
                        Full access to all modules: Overview, Publishers (Add/Import/Edit), Groups, Emergency Contacts, Reports, Schedules, Territory, and Settings.
                      </div>
                    </div>
                  </div>
                </v-card>

                <!-- Admin (Service Committee) -->
                <v-card
                  variant="outlined"
                  class="pa-3 cursor-pointer transition-swing"
                  :class="{ 'border-indigo bg-indigo-lighten-5': activeRolePreset === 'admin_sc' }"
                  @click="applyRolePreset('admin_sc')"
                >
                  <div class="d-flex align-start">
                    <v-checkbox-btn
                      :model-value="activeRolePreset === 'admin_sc'"
                      color="indigo"
                      density="compact"
                      class="mt-0 pt-0 mr-2"
                      @update:model-value="applyRolePreset('admin_sc')"
                    />
                    <div class="flex-grow-1">
                      <div class="d-flex align-center justify-space-between">
                        <span class="text-subtitle-2 font-weight-bold">Admin (Service Committee)</span>
                        <v-chip size="x-small" color="indigo" variant="flat">Delete Privilege</v-chip>
                      </div>
                      <div class="text-caption text-medium-emphasis mt-1">
                        Full Admin access + exclusive permission to permanently delete records and groups. Granted to appointed Coordinator, Secretary, or Service Overseer.
                      </div>
                    </div>
                  </div>
                </v-card>
                ...
                <v-card
                  variant="outlined"
                  class="pa-3 cursor-pointer transition-swing"
                  :class="{ 'border-blue bg-light-blue-lighten-5': activeRolePreset === 'editor_ms' }"
                  @click="applyRolePreset('editor_ms')"
                >
                  <div class="d-flex align-start">
                    <v-checkbox-btn
                      :model-value="activeRolePreset === 'editor_ms'"
                      color="blue"
                      density="compact"
                      class="mt-0 pt-0 mr-2"
                      @update:model-value="applyRolePreset('editor_ms')"
                    />
                    <div class="flex-grow-1">
                      <div class="d-flex align-center justify-space-between">
                        <span class="text-subtitle-2 font-weight-bold">Editor (Ministerial Servant)</span>
                        <v-chip size="x-small" color="blue" variant="flat">MS Group Editor</v-chip>
                      </div>
                      <div class="text-caption text-medium-emphasis mt-1">
                        Home & Congregation Overview view-only (cards non-clickable), last month summary for assigned Group. Publishers List (view-only, no Add/Import/Edit) and Emergency Contacts restricted strictly to assigned Group. Groups menu hidden, Groups List order editing disabled. Publisher Record edit disabled. Territory List for assigned Group (Overview & S-13 hidden unless Territory Assistant).
                      </div>
                    </div>
                  </div>
                </v-card>
                ...
                <v-card
                  variant="outlined"
                  class="pa-3 cursor-pointer transition-swing"
                  :class="{ 'border-teal bg-teal-lighten-5': activeRolePreset === 'editor_pub' }"
                  @click="applyRolePreset('editor_pub')"
                >
                  <div class="d-flex align-start">
                    <v-checkbox-btn
                      :model-value="activeRolePreset === 'editor_pub'"
                      color="teal"
                      density="compact"
                      class="mt-0 pt-0 mr-2"
                      @update:model-value="applyRolePreset('editor_pub')"
                    />
                    <div class="flex-grow-1">
                      <div class="d-flex align-center justify-space-between">
                        <span class="text-subtitle-2 font-weight-bold">Editor (Publisher)</span>
                        <v-chip size="x-small" color="teal" variant="flat">Publisher Editor</v-chip>
                      </div>
                      <div class="text-caption text-medium-emphasis mt-1">
                        Home & Congregation Overview view-only (cards non-clickable), last month summary for assigned Group. Publishers List is completely hidden. Emergency Contacts restricted to assigned Group. Groups menu hidden, Groups List order editing disabled. Publisher Record edit disabled. Territory List for assigned Group (Overview & S-13 hidden unless Territory Assistant).
                      </div>
                    </div>
                  </div>
                </v-card>
                ...
                <v-card
                  variant="outlined"
                  class="pa-3 cursor-pointer transition-swing"
                  :class="{ 'border-grey bg-grey-lighten-4': activeRolePreset === 'viewer' }"
                  @click="applyRolePreset('viewer')"
                >
                  <div class="d-flex align-start">
                    <v-checkbox-btn
                      :model-value="activeRolePreset === 'viewer'"
                      color="grey"
                      density="compact"
                      class="mt-0 pt-0 mr-2"
                      @update:model-value="applyRolePreset('viewer')"
                    />
                    <div class="flex-grow-1">
                      <div class="d-flex align-center justify-space-between">
                        <span class="text-subtitle-2 font-weight-bold">Viewer</span>
                        <v-chip size="x-small" color="grey-darken-1" variant="flat">Read Only</v-chip>
                      </div>
                      <div class="text-caption text-medium-emphasis mt-1">
                        Read-only access for viewing basic meeting schedules. Cannot access publisher data, personal records, or settings.
                      </div>
                    </div>
                  </div>
                </v-card>
...
const activeRolePreset = computed(() => {
  if (userForm.value.role === 'admin') {
    const uUid = userForm.value.uid || userForm.value.id
    const isSC = congSettings.value && (
      congSettings.value.coordinatorUid === uUid ||
      congSettings.value.secretaryUid === uUid ||
      congSettings.value.serviceOverseerUid === uUid ||
      (congSettings.value.coordinator && congSettings.value.coordinator.toLowerCase() === userForm.value.displayName?.toLowerCase()) ||
      (congSettings.value.secretary && congSettings.value.secretary.toLowerCase() === userForm.value.displayName?.toLowerCase()) ||
      (congSettings.value.serviceOverseer && congSettings.value.serviceOverseer.toLowerCase() === userForm.value.displayName?.toLowerCase())
    )
    return isSC ? 'admin_sc' : 'admin'
  }
  if (userForm.value.role === 'editor') {
    if (userForm.value.spiritualRole === 'Ministerial Servant') return 'editor_ms'
    return 'editor_pub'
  }
  if (userForm.value.role === 'viewer') return 'viewer'
  return ''
})
...
const applyRolePreset = (preset) => {
  if (preset === 'admin') {
    ...
    userForm.value.spiritualRole = 'Elder'
  } else if (preset === 'admin_sc') {
    userForm.value.role = 'admin'
    userForm.value.spiritualRole = 'Elder'
  } else if (preset === 'editor_ms') {
    userForm.value.role = 'editor'
    userForm.value.spiritualRole = 'Ministerial Servant'
  } else if (preset === 'editor_pub') {
    userForm.value.role = 'editor'
    userForm.value.spiritualRole = 'Publisher'
  } else if (preset === 'viewer') {
    userForm.value.role = 'viewer'
    userForm.value.spiritualRole = 'Publisher'
  }
}
